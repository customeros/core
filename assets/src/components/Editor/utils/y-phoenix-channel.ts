...
const setupChannel = (provider: PhoenixChannelProvider) => {
  if (provider.shouldConnect && (provider.channel == null || provider.channel.state === 'closed')) {
    // Clean up existing channel if it exists
    if (provider.channel) {
      provider.channel.leave();
    }

    provider.channel = provider.socket.channel(provider.roomname, provider.params);

    provider.channel.onError(error => {
 ...
 *
 * @extends {ObservableV2<{'connection-close': (event: CloseEvent | null,  provider: PhoenixChannelProvider) => any, 'status': (event: { status: 'connected' | 'disconnected' | 'connecting' }) => any, 'connection-error': (event: Event, provider: PhoenixChannelProvider) => any, 'sync': (state: boolean) => any, 'synced': (state: boolean) => any }>}
 */
export class PhoenixChannelProvider extends ObservableV2<{
  ...
  constructor(
    ...
    this._updateHandler = (update, origin) => {
      // Only broadcast updates that didn't originate from this client
      // This ensures we don't create feedback loops
      if (origin !== this) {
        const encoder = encoding.createEncoder();
    ...
    // Listen to all updates, including undo/redo operations
  ...
  connect() {
    ...
    if (this.channel == null || this.channel.state === 'closed') {
      setupChannel(this);
      this.connectBc();
    }
  }
