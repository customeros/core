defmodule Core.Integrations.OAuth.Token do
  ...
  def new(%{"access_token" => access_token} = data) when is_binary(access_token) do
    with {:ok, expires_at} <- get_expires_at(data),
         ...
         {:ok, refresh_token} <- get_refresh_token(data),
         {:ok, scopes} <- get_scopes(data) do
      {:ok,
       %__MODULE__{
  ...
  defp get_scopes(%{"scope" => scope}) when is_binary(scope), do: {:ok, String.split(scope, " ")}
  defp get_scopes(_), do: {:ok, []}
end
